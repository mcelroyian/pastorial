...
class Intent(ABC):
    ...
    """
    def __init__(self, task_id: Optional[uuid.UUID] = None): # Added task_id
        self.intent_id: uuid.UUID = uuid.uuid4()
        self.status: IntentStatus = IntentStatus.PENDING
        self.error_message: Optional[str] = None
        self.originating_task_id: Optional[uuid.UUID] = task_id # Store originating task_id
...
class GatherIntent(Intent): # Will likely be replaced by InteractAtTargetIntent + task logic
    """Intent for an agent to gather a resource from a target node."""
    def __init__(self, resource_node_id: uuid.UUID, resource_type: Any, quantity_to_gather: int, task_id: Optional[uuid.UUID] = None): # Using Any for ResourceType for now
        super().__init__(task_id)
...
class DeliverIntent(Intent): # Will likely be replaced by InteractAtTargetIntent + task logic
    """Intent for an agent to deliver a resource to a target dropoff."""
    def __init__(self, dropoff_id: uuid.UUID, resource_type: Any, quantity_to_deliver: int, task_id: Optional[uuid.UUID] = None): # Using Any for ResourceType
        super().__init__(task_id)
...
class RandomMoveIntent(Intent):
    """Intent for an agent to move to a random position."""
    def __init__(self, task_id: Optional[uuid.UUID] = None): # task_id will likely be None for this
        super().__init__(task_id)
...
class GatherAndDeliverFullLoadIntent(Intent): # This is likely a task-level concept, not a direct agent intent
    """
    ...
    """
    def __init__(self, resource_node_id: uuid.UUID, resource_type: Any, dropoff_id: uuid.UUID, task_id: Optional[uuid.UUID] = None):
        super().__init__(task_id)
        self.resource_node_id: uuid.UUID = resource_node_id
