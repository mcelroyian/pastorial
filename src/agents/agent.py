...
class Agent:
    ...
    def _process_current_intent(self):
        ...
        self.logger.debug(f"Processing current intent: Intent type determined as {intent_type}")

        if intent_type == MoveIntent or intent_type == RandomMoveIntent:
            self.logger.debug(f"Matched MoveIntent or RandomMoveIntent. Transitioning to MovingBehavior.")
            ...
            self.logger.debug(f"Matched Gather/Deliver/InteractAtTargetIntent group. Current intent actual type: {type(self.current_intent)}")
            if isinstance(self.current_intent, (GatherIntent, DeliverIntent, InteractAtTargetIntent)):
                 self.logger.debug(f"isinstance check PASSED for InteractingBehavior. Transitioning to InteractingBehavior with intent: {self.current_intent}")
                 self._transition_behavior(InteractingBehavior, self.current_intent)
            else: # Should not happen if intent_type matched
                 self.logger.error(f"Unhandled intent type {intent_type} after type check (isinstance FAILED). Intent: {self.current_intent}")
            ...
            self.logger.warning(f"Unknown intent type {intent_type}. Intent: {self.current_intent}. Transitioning to IdleBehavior.")
    ...
    def _follow_path(self, dt: float) -> bool:
        ...
        if not self.current_path or not self.target_position: # target_position is the current waypoint
            self.logger.debug(f"Follow_path: No current_path or target_position. Path: {self.current_path}, Target: {self.target_position}. Returning False.")
        ...
        self.logger.debug(f"Follow_path: Current pos: {self.position}, Waypoint: {self.target_position}, Dist: {distance_to_waypoint:.2f}, Path: {self.current_path}")

        if distance_to_waypoint < self.target_tolerance:
            # Reached the current waypoint
            self.logger.debug(f"Follow_path: Reached waypoint {self.target_position} (within tolerance). Snapping position.")
            ...
            if self.current_path: # Should always be true if target_position was set from it
                ...
                self.logger.debug(f"Follow_path: Popped waypoint. Remaining path: {self.current_path}")
            ...
            if not self.current_path: # Path is now empty, meaning final destination of this path segment reached
                ...
                self.logger.debug(f"Follow_path: Reached end of path (path empty after pop).")
                ...
                self.target_position = self.current_path[0]
                self.logger.debug(f"Follow_path: New waypoint set: {self.target_position}. Path left: {self.current_path}")
            ...
            self.logger.debug(f"Follow_path: Moving towards {self.target_position}. Movement: {potential_movement} (Speed: {self.speed}, dt: {dt})")
            
            if potential_movement.length() >= distance_to_waypoint:
                # Will reach or overshoot waypoint this frame
                self.logger.debug(f"Follow_path: Will reach/overshoot waypoint {self.target_position} this frame. Snapping.")
                self.position = pygame.math.Vector2(self.target_position) # Snap to waypoint
                
                if self.current_path: self.current_path.pop(0)
                self.logger.debug(f"Follow_path: Popped waypoint after moving. Remaining path: {self.current_path}")

                if not self.current_path:
                    self.target_position = None
                    self.final_destination = None
                    self.logger.debug(f"Follow_path: Reached end of path by moving (path empty after pop).")
                    # print(f"DEBUG: Agent {self.id} reached end of path by moving.")
                    return True # Final destination of path segment reached
                else:
                    self.target_position = self.current_path[0]
                    self.logger.debug(f"Follow_path: New waypoint set after moving: {self.target_position}")
                ...
                self.logger.debug(f"Follow_path: Moved. New position: {self.position}")
        
        self.logger.debug(f"Follow_path: Waypoint {self.target_position} not yet reached. Returning False.")
    ...
    def update(self, dt: float, resource_manager: 'ResourceManager'):
        ...
        self.logger.debug(f"Update START. Behavior: {self.current_behavior}, Intent: {self.current_intent.intent_id if self.current_intent else 'None'}")
        ...
        if not self.current_behavior:
            self.logger.error(f"Has no current_behavior. This should not happen. Defaulting to IdleBehavior.")
        ...
        if intent_status_update is not None: # Behavior signals an intent it was handling has finished
            if self.current_intent:
                ...
                log_message = f"Intent {self.current_intent.intent_id} ({self.current_intent.get_description()}) outcome: {intent_status_update.name}."
                ...
                if intent_status_update == IntentStatus.FAILED:
                    ...
                    if self.current_intent.originating_task_id:
                        ...
                        self.logger.debug(f"Notifying TaskManager about intent {self.current_intent.intent_id} outcome for task {originating_task_id_of_intent}.")
                        ...
                        self.logger.debug(f"Post-notify: Task ID {originating_task_id_of_intent}. Retrieved task_object: {'Exists' if task_object else 'None'}. Intent outcome: {intent_status_update.name if intent_status_update else 'N/A'}")
                        if task_object:
                            self.logger.debug(f"Post-notify: Task {task_object.task_id} current status from agent's perspective: {task_object.status.name if task_object.status else 'N/A'}")
                            if task_object.status == TaskStatus.COMPLETED or task_object.status == TaskStatus.FAILED:
                                self.logger.info(f"Task {task_object.task_id} (Type: {task_object.task_type.name}) has reached terminal state: {task_object.status.name} after intent {self.current_intent.intent_id} outcome.")
                            ...
                            self.logger.warning(f"Could not retrieve task {originating_task_id_of_intent} after intent outcome to check its final status.")
    
                    else: # Intent exists but no originating task (e.g. RandomMoveIntent)
                        self.logger.debug(f"Intent {self.current_intent.intent_id} (type: {type(self.current_intent)}) finished with {intent_status_update.name}, but no originating_task_id. No task to notify or report outcome for.")
                    ...
                    if intent_status_update == IntentStatus.FAILED:
                        # If an intent fails, the responsible behavior (e.g., PathFailedBehavior) has already run.
                        # The task has been notified. If the task is now terminal, the agent should seek new work.
                        # If the task is not terminal (e.g., it will retry), it should submit a new intent.
                        # In either case, the agent's role with the FAILED intent is done. It should clear
                        # the intent and move to EvaluatingIntentBehavior to see what's next.
                        ...
                        if isinstance(self.current_intent, GatherIntent):
                            ...
                            if self.current_inventory['resource_type'] is None or self.current_inventory['resource_type'] == resource_type:
                                ...
                                self.logger.info(f"Inventory updated: +{gathered_qty} of {resource_type}. New total: {self.current_inventory['quantity']}")
                            else:
                                self.logger.error(f"Inventory mismatch after GatherIntent. Has {self.current_inventory['resource_type']}, gathered {resource_type}")
                            ...
                            self.logger.info(f"Inventory updated: -{delivered_qty}. New total: {self.current_inventory['quantity']}")
                        ...
                        # Transitioning to EvaluatingIntentBehavior will handle processing this new/pending intent.
                             ...
                             self.logger.debug(f"Intent {completed_intent_id} COMPLETED, but task {originating_task_id_of_intent} not yet terminal. Task will submit next intent. Current agent intent: {self.current_intent.intent_id if self.current_intent else 'None'}")
                             # Clear the completed intent so the agent waits for the next one from the task
                             if self.current_intent and self.current_intent.intent_id == completed_intent_id:
                                 self.current_intent = None
                             self._transition_behavior(EvaluatingIntentBehavior)
                        ...
                        self.logger.info(f"Intent {completed_intent_id} was cancelled.")
                        ...
                        self.logger.error(f"Behavior {self.current_behavior} reported intent status but no current_intent was set on agent.")
        ...
        if not self.current_intent and not isinstance(self.current_behavior, EvaluatingIntentBehavior) and not isinstance(self.current_behavior, IdleBehavior) :
            self.logger.debug(f"Has no intent and is in behavior {self.current_behavior}. Transitioning to EvaluatingIntentBehavior to find work.")
            self._transition_behavior(EvaluatingIntentBehavior)
